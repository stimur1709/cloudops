...
public enum TaskCapabilityReason {
    UNSUPPORTED_RESOURCE_TYPE("RESOURCE_UNSUPPORTED", "Task is not supported by the resource type"), RESOURCE_INACTIVE(
            "RESOURCE_INACTIVE",
            "Task requires an active resource"),
    SSH_ENDPOINT_NOT_CONFIGURED("SSH_ENDPOINT_NOT_CONFIGURED",
            "SSH endpoint is not configured for the resource"),
    SSH_CREDENTIAL_NOT_CONFIGURED(
            "SSH_CREDENTIAL_NOT_CONFIGURED",
            "SSH credential is not configured for the resource"),
