...
public class JpaSearchService {
    ...
    public <E> SearchResult<E> search(
        ...
        Long total = search.getTotal() ? count(builder, scope, preparedSearch.filter(), definition) : null;
    ...
    private <E> PreparedCondition<E> prepareCondition(
        ...
        if (!field.supports(condition.operation())) {
            throw invalid(
                    path + ".operation",
                    "Operation %s is not supported for field %s".formatted(condition.operation(), condition.field()));
        }
    ...
    private <E> List<PreparedSort<E>> prepareSort(List<SearchQuery.Sort> sort, JpaSearchDefinition<E> definition) {
        List<PreparedSort<E>> prepared = new ArrayList<>(IntStream.range(0, sort.size())
                ...
                .toList());
        boolean hasDefaultSort = prepared.stream().anyMatch(item -> item.name().equals(definition.defaultSortField()));
