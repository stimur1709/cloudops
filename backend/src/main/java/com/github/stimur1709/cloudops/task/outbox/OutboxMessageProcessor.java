    ...
    OutboxProcessingResult process(UUID messageId) {
        ...
        try {
            boolean confirmed = publisher.publish(
                    message.id(),
                    new TaskExecutionCommand(
                            message.payload().required("taskId").asLong()));
            if (!confirmed) {
                log.warn(
                        "Outbox publication was not confirmed: messageId={}, messageType={}, aggregateId={}",
                        message.id(),
                        message.messageType(),
                        message.aggregateId());
            ...
            repository.flush();
            log.info(
                    "Published outbox message: messageId={}, messageType={}, aggregateId={}",
                    message.id(),
                    message.messageType(),
                    message.aggregateId());
            ...
            log.warn(
                    "Failed to publish outbox message: messageId={}, messageType={}, aggregateId={}, reason={}",
                    message.id(),
                    message.messageType(),
                    message.aggregateId(),
