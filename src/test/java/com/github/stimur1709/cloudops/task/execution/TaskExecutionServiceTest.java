...
class TaskExecutionServiceTest {
    ...
    void exhaustsRetryAndReturnsDeadLetterOutcome() {
        ...
        verify(persistence)
                .fail(7, EXECUTION_ID, TaskErrorCode.RETRY_EXHAUSTED, "Task execution failed after retry attempts");
    }
    ...
    void doesNotRetryUnclassifiedRuntimeException() {
        ...
        verify(handler).execute(any());
        verify(persistence).fail(7, EXECUTION_ID, TaskErrorCode.EXECUTION_ERROR, "Task execution failed unexpectedly");
    }
    ...
    void controlledNegativeResultIsAcknowledgedWithoutRetry() {
        arrangeClaimedTask();
        when(handler.execute(any()))
                .thenReturn(TaskExecutionResult.failed(TaskErrorCode.EXECUTION_ERROR, "Operation failed"));
    ...
    void turnsMissingHandlerIntoControlledDeadLetterFailure() {
        ...
        verify(persistence).fail(7, EXECUTION_ID, TaskErrorCode.HANDLER_NOT_FOUND, "Task handler is not configured");
    ...
    void acknowledgesExecutionFencedBeforeAttempt() {
        arrangeClaimedTask();
        when(persistence.recordAttempt(7, EXECUTION_ID)).thenThrow(new StaleTaskExecutionException(7, EXECUTION_ID));
    ...
    void acknowledgesExceptionFromHandlerWhenFailureUpdateIsFenced() {
        ...
        when(persistence.fail(7, EXECUTION_ID, TaskErrorCode.EXECUTION_ERROR, "Task execution failed unexpectedly"))
                .thenReturn(false);
    ...
    void doesNotRerunHandlerWhenTerminalResultCannotBeSaved() {
        ...
        doThrow(new IllegalStateException("database unavailable"))
                .when(persistence)
                .complete(eq(7L), eq(EXECUTION_ID), any());
    ...
    private void arrangeClaim() {
        when(persistence.claim(7))
                .thenReturn(new TaskPersistenceService.ClaimedTask(
                        7,
                        8,
