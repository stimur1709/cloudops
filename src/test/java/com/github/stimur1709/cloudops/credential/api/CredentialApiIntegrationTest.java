...
class CredentialApiIntegrationTest {
    ...
    @Autowired
    SqlStatementRecorder statementRecorder;
    ...
    void setUp() {
        ...
        organizationId = jdbc.queryForObject(
                "INSERT INTO organizations (name,created_at,updated_at) VALUES ('Org',now(),now()) RETURNING id",
                Long.class);
        jdbc.update(
                "INSERT INTO organization_memberships (organization_id,user_id,role,created_at,updated_at) VALUES (?,?,'OWNER',now(),now())",
                organizationId,
                TestAuthentication.USER_ID);
        resourceId = jdbc.queryForObject(
                "INSERT INTO resources (name,type,status,organization_id,config,created_at,updated_at) VALUES ('server','SERVER','ACTIVE',?,'{}',now(),now()) RETURNING id",
                Long.class,
                organizationId);
    }
    ...
    void crudNeverReturnsSecretAndStoresOnlyCiphertext() throws Exception {
        ...
        mockMvc.perform(get("/api/credentials/{id}", id))
    ...
    void bindsCompatibleCredentialAndBlocksDeletingIt() throws Exception {
        ...
        mockMvc.perform(delete("/api/credentials/{id}", id))
    ...
    void getsAllCredentialMetadataWithoutQueriesInLoop() throws Exception {
        ...
        assertThat(relevantSelects.stream().filter(sql -> sql.contains(" join credentials ")))
                .hasSize(1);
        assertThat(relevantSelects.stream().filter(sql -> sql.contains(" from credentials ")))
                .isEmpty();
    }
