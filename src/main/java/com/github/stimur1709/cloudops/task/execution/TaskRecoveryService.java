...
public class TaskRecoveryService {
    ...
    public int recoverExpired() {
        var tasks = taskRepository.lockExpired(
                clock.instant(), PageRequest.of(0, properties.recoveryBatchSize())
        );
    ...
    private void recover(TaskEntity task) {
        if (task.recoveryCount() >= properties.maxRecoveries()) {
            ...
            log.warn("event=task_recovery_exhausted taskId={} recoveryCount={}",
                    task.id(), task.recoveryCount());
            return;
        }
        ...
        log.info("event=recovery_outbox_message_created taskId={} messageId={} deduplicationKey={}",
                task.id(), message.id(), message.deduplicationKey());
    }
}
