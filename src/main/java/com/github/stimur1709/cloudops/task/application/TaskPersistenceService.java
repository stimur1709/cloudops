...
public class TaskPersistenceService {
    ...
    public ClaimedTask claim(long taskId) {
        ...
        int updated = taskRepository.claimPending(
                taskId, now, executionId, now.plus(leaseProperties.duration()), TaskStatus.PENDING, TaskStatus.RUNNING);
        if (updated == 0) {
            return null;
        }
        ...
        log.info(
                "event=task_lease_acquired taskId={} executionId={} leaseExpiresAt={}",
                taskId,
                executionId,
                task.leaseExpiresAt());
