...
public class CredentialService {
    ...
    public CredentialService(CredentialJpaRepository repository, ResourceCredentialJpaRepository bindingRepository,
            OrganizationJpaRepository organizationRepository, OrganizationAuthorization authorization,
            JpaSearchService searchService, SecretCryptoService cryptoService, Clock clock) {
    ...
    @Transactional
    public CredentialEntity create(long organizationId, String name, CredentialType type, String username,
            String secret, long userId) {
        ...
        return save(CredentialEntity.create(organization, name, type, username,
                cryptoService.encrypt(secret), clock.instant()));
    }
    ...
    @Transactional
    public CredentialEntity update(long id, String name, CredentialType type, String username, String secret,
            long userId) {
        ...
        if (type == CredentialType.SSH_PRIVATE_KEY && bindingRepository.findAllByCredentialId(id).stream()
                .anyMatch(binding -> binding.purpose() == CredentialPurpose.DATABASE)) {
            throw new BadRequestException("INCOMPATIBLE_CREDENTIAL",
                    "Credential type is not compatible with an existing resource binding");
        }
    ...
    private CredentialEntity save(CredentialEntity entity) {
        ...
        catch (DataIntegrityViolationException exception) { throw nameConflict(); }
    }

    private ConflictException nameConflict() {
        return new ConflictException("CREDENTIAL_NAME_CONFLICT",
                "Credential name is already used in this organization");
    }
}
