...
Повторяемые summary-метрики одного контекста объединяются в один нейтральный summary panel/grid. Например, Availability, Uptime и Coverage располагаются тремя колонками внутри одного блока с общей рамкой; не создавать отдельную полноширинную card для каждого числа. На узкой ширине колонки складываются вертикально без потери подписей и пояснений.
...
| **SelectedEntityHistory** | Выбранная entity, server pagination/sorting, loading/empty/unavailable/error и rows | Одна detail-area после списка; заголовок всегда называет выбранную entity. Недоступная history скрывает ненужные sort/page controls и не ломает основной overview |
