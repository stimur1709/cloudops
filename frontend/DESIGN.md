...
Resource details: сводка имени и health; вкладки Overview, Health, Monitoring, Tasks, History, Settings по реально доступным данным. Вкладки — независимые контексты: одновременно отображается только активная tab panel, а не несколько длинных секций с anchor-navigation. Активная вкладка имеет явный accent indicator, `aria-selected` и стандартную клавиатурную модель tabs. Выбор вкладки сериализуется в URL и сохраняет organization/resource context и остальные применимые query parameters. Связанные данные открываются по ссылке; Back восстанавливает фильтры, страницу и по возможности позицию списка. Search, sort, filters, page и time range должны иметь сериализуемое URL-состояние. Названия маршрутов согласуются при реализации; отдельный router не предписывается выбранным стеком.

Повторяемые summary-метрики одного контекста объединяются в один нейтральный summary panel/grid. Например, Availability, Uptime и Coverage на desktop располагаются **одной строкой в трёх колонках** внутри одного блока с общей рамкой; не создавать отдельную полноширинную card для каждого числа и не складывать метрики вертикально при доступной desktop-ширине. На узкой ширине колонки складываются вертикально без потери подписей и пояснений. Использовать реально определённые проектом брейкпоинты `md`/`lg`, а не отсутствующий `sm`.

Monitoring overview на desktop использует плотную таблицу/список с колонками type, health, last checked, next run, last result и actions. Большая отдельная card на каждый Monitor запрещена. На mobile строки преобразуются в компактные MonitorCard, сохраняя type, health, freshness, result и действия. Технические поля и raw enum находятся в компактном раскрываемом details, а не в основном списке. Выбранная строка получает `accent-soft` и явное `aria-selected`; history выбранного Monitor открывается одной самостоятельной detail-area после списка и не расширяет каждую строку.

Monitoring и History rows по умолчанию компактны: подтверждённый результат, короткое время и при наличии один полезный типизированный показатель видны без раскрытия. Secondary technical data показываются только в отдельном disclosure; внутри — плотный responsive key/value grid, raw identifiers/debug metadata визуально вторичны. History использует ту же typography/density и компактное время, что Monitoring; полный timestamp с timezone доступен в tooltip/деталях. Одностраничная history может показывать диапазон без disabled pager. Подтверждение нового результата manual run не остаётся постоянным текстом под кнопкой после обновления строки.
...
| **MonitorTable / MonitorCard** | Monitor type, health, lastCheckedAt, nextRunAt, lastResult; effective settings отдельно при наличии | Desktop table использует короткое пользовательское имя типа и компактные значения; mobile card содержит тот же минимальный набор. Raw type и ID доступны в details. ProbeStatus отделяет health от execution feedback. Enabled/disabled — отдельная нейтральная настройка, не выводится из nextRunAt=null |
| **SelectedEntityHistory** | Выбранная entity, server pagination/sorting, loading/empty/unavailable/error и rows | Одна detail-area после списка; заголовок всегда называет выбранную entity. Строка по умолчанию показывает время, результат и максимум один полезный показатель; technical grid раскрывается отдельно. Недоступная history скрывает ненужные sort/page controls и не ломает основной overview |
| **TechnicalDataGrid** | Типизированные пары label/value, необязательные raw metadata | Общая компактная двухколоночная desktop и одноколоночная mobile сетка с inline label/value; длинные URL/host переносятся внутри области. Raw ID/enum отделены и приглушены. Используется внутри disclosure, не разворачивается автоматически |
